<div class="vignette album-vignette">
    @if (isset($score) && (bool)$score)
        <div class="badges clear">
            @include('components.scores.pct-badge', ['label' => "Global", 'percent' => $album->globalScore()])
            @if(!is_null(auth()->user()))
                @include('components.scores.pct-badge', ['label' => "Subs", 'percent' => $album->subsScore(auth()->user())])
            @endif
        </div>
    @endif
    <div class="picture">
